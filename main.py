...
TWEET_STOCK = [
    "子どもの習い事チラシをLINEに送ったら、AIが日時を読み取ってGoogleカレンダーに自動登録してくれた🙌 もう手入力しなくていい。 #AI副業 #ワーママ #LINE",
    ...
    "学校のプリント管理もLINEボットに追加。写真送ったら内容を読み取って管理してくれる📄 #AI副業 #ワーママ #LINE",
    ...
    "Famm締切リマインダーを追加。毎月忘れてたのがゼロになった。小さい自動化ほど日常が楽になる #AI副業 #ワーママ #自動化",
    ...
    "薬膳ブログの記事公開時にInstagram用の3枚スライドが自動生成されるようにした📸 1枚目:料理写真 2枚目:使う食材 3枚目:効能まとめ。あとはLINEから保存してカルーセル投稿するだけ #AI副業 #Instagram #自動化",
    "医師ママが実践する薬膳レシピのInstagramアカウント（@foodmakehealth）を開設した🌿 医療職という肩書きを前面に出すとコンセプトが一気に明確になった #AI副業 #薬膳 #Instagram",
    "薬膳記事のAmazonアフィリエイトを自動挿入する仕組みを作った📚 記事内容を見て「子ども向け→こども薬膳」「スープ系→スープジャー弁当」「それ以外→ずぼら薬膳」と自動で判定して挿入。貼り忘れゼロ #AI副業 #アフィリエイト #自動化",
    "薬膳ブログのリライト、記事URLをLINEに貼るだけで特定の記事だけリライトできるようになった🔄 「2（リライト）→URL貼り付け」の2ステップ。自動選択か指定かを選べる #AI副業 #ブログ自動化 #LINE",
    ...
    # ── eBayリサーチAI進化 ──
    "LINEに「ebay 風呂敷」と送るだけでAIがeBayキーワード10個を自動生成して並列検索してくれる仕組みを作った📦 「軽い」「小さい」とか日本語で条件指定してもちゃんと英語キーワードに変換してくれる #AI副業 #eBay #物販",
    "eBayリサーチに「ウォッチャー数」を追加した👀 競合が少なくて平均価格が高くてウォッチが多い＝需要ある穴場、という判定ロジック。売れ筋に近い指標がLINEで届くようになった #AI副業 #eBay #物販",
    "eBayリサーチの結果に「次のステップ」を追加した💡 ①キーワードで検索②Sold Listingsで絞る③30日の売れ数を確認→仕入れ。AIが出した候補を自分の目で最終確認するフローが大事 #AI副業 #eBay #物販",
    ...
    "課題の競合分析データ、スプレッドシートに自動書き込みできた📊 Sheets APIの有効化→認証→書き込みまで全部Claude Codeと一緒にやったら10分で完成。コピペ作業がゼロになった #AI副業 #ClaudeCode #自動化",
    "ウェブマーケティングの課題でLP競合分析をClaude Codeに頼んだら3社分のデータ取得→整形→スプレッドシート書き込みまで全自動だった😳 勉強のための作業すらAIに任せられる時代 #AI副業 #ClaudeCode #Webマーケティング",
    # ── インスタ×SNS自動化挑戦 ──
    "インスタを始めることにした📱 AI副業自動化のジャンルで。顔出しなし・バズ狙いなし・全自動で回す設計中。まず1on1でアカウント設計から。 #AI副業 #インスタ #自動化",
    "インスタのリール、冒頭2秒が全てと知った。視聴者が「自分ごと」と思うかどうかが勝負。王道は共感型：「〇〇で悩んでる人へ」。医療職ママのAI副業話、刺さる人には刺さるはず #AI副業 #インスタ #リール",
    "インスタのプロに聞いた話：ジャンルを絞らずに2年・600投稿してもフォロワー50人。絞ったら4ヶ月で5万人。やることより「何をやらないか」を決める方が大事だった #AI副業 #インスタ",
    "Claude Codeでインスタ投稿も自動化したい。テキスト生成→AI画像生成→動画化→投稿まで全部つなげる計画。まずMeta APIの審査を通すのが先だけど😅 #AI副業 #自動化 #ClaudeCode",
    "インスタで収益化するには「売らない」が正解と知った。価値を提供しまくって、欲しいと思った人だけに案内する。セールスより価値提供。これLINEボットでやってることと同じだ #AI副業 #インスタ #マーケティング",
    # ── コピーライティング・note ──
