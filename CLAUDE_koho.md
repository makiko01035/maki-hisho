...
### リプライの口調ガイドライン（キャラブレ防止）
自動投稿は「体験を語る実録系」のトーンで統一されている。手動リプライもこれに合わせる。

**NG口調（投稿とのキャラ乖離）**
- 「ありがとうございます！」→ 感嘆符・テンション高めはブレる
- 「そうなんです〜😊」→ 絵文字多用・語尾伸ばしはブレる
- 「私も頑張ります！」→ 「これからやる人」目線はブレる（まきは「やってきた人」）

**OK口調（まきキャラの維持）**
- 「ありがとうございます。励みになります。」
- 「そうなんです。〇〇が特に変わりました。」
- 「私も同じでした。〇〇で少し楽になりました。」
- 「やってみたら意外と〇〇でした。」

**ポイント2つ**
1. 「体験を語る人」ポジションを崩さない（共感しつつ自分の実録に引き戻す）
2. 語尾は「〜ました。〜です。」で統一。感嘆符・絵文字は控えめに
...
- **稼働中（2026-05-18〜）**
- `KOHARU_THREADS_ACCESS_TOKEN` / `KOHARU_THREADS_USER_ID`（26927926833493808）Render設定済み
- トークン取得：Meta開発者ダッシュボード → koharu-mako-threads → Threads API → 設定 → ユーザートークン生成ツール → koharusroomとして続行
